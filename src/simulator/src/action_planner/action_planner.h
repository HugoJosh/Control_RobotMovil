...
bool action_planner(float px, float py, float theta, movement *movements, float *x_nP, float *y_np);
void clips(float px, float py, float theta, movement *movements);
...
bool action_planner(float px, float py, float theta, movement *movements, float *x_nP, float *y_np){
  ...
  if(init_flg==1){
   ...
   // It starts the communication with the Clips node LO HACE UNA SOLA VEZ
   start_clips_node_action_planner();
   init_flg=0;
   strcpy(arm,"manipulator");
   SimuladorRepresentation::strQueryKDB("(assert (start action-planning))", result, 10000); //Guarda en el result la respuesta
   //DENTRO DEL RESULT LLEGA CUAL ES LA ACCIÓN A REALIZAR

   std::cout << "CLIPS answer: " << result << std::endl; //La lanza a la terminal
  }

  i++;
  sprintf(str,"(assert (step %d ))",i); //Agrega el nuevo hecho paso i-ésimo
  printf("\nSend fact %s\n",str);
  SimuladorRepresentation::strQueryKDB(str, result, 10000);
  printf("\nCLIPS answer: %d %s\n",i,result.c_str()); //Le responde qué hacer?
  //LA SEGUNDA FRASE DE EL RESULTADO ES LA ACCIÓN QUE SE REALIZARÁ
  ...
  if(strcmp(action,"goto")==0){
   ...
   //Me dice a donde está el siguiente objetivo
